...
elploteo_02 <- function(infile, N, miRNAs.table){
	...
	# Plotting data from the first row of DATA.
	print("Plotting DATA...")
	plot(x, yp, type="l", ylim=c(as.numeric(min_results[1]) - 5, as.numeric(max_results[1]) + 20), ylab="P(X)", xlab=METADATA[2], main=METADATA[1], col=ifelse(rownames(DATA)[1] %in% topNmirnas, miRNAs.table[miRNAs.table$sequence == rownames(DATA)[1], "color"], "gray"), cex.main=1)
#	plot(x, yp, type="l", ylim=c(as.numeric(min_results[1]) - 5, as.numeric(max_results[1]) + 20), ylab="P(X)", xlab=METADATA[2], main=METADATA[1], col=col1, cex.main=1)
	...
	for(i in 2:length(DATA[,1])){
		lines(x, as.vector(DATA[i, ]), col=ifelse(rownames(DATA)[i] %in% topNmirnas, miRNAs.table[miRNAs.table$sequence == rownames(DATA)[i], "color"], "gray"))
	...
	print(topNmirnas)
	print(miRNAs.table$sequence)
	print(rownames(miRNAs.table))
	print(rownames(miRNAs.table[miRNAs.table$sequence %in% topNmirnas,]))
	miRNAs.table[miRNAs.table$sequence %in% topNmirnas, "color"]
	legend("topright", rownames(miRNAs.table[miRNAs.table$sequence %in% topNmirnas,]), col=miRNAs.table[miRNAs.table$sequence %in% topNmirnas, "color"], lty=1)
}
